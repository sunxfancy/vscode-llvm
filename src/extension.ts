...
export function activate(context: vscode.ExtensionContext) {
	...
	let open_settings = vscode.commands.registerCommand('llvmPipelineView.settings', async () => {
		// The code you place here will be executed every time your command is executed
		// Display a message box to the user
		vscode.window.showInformationMessage('llvmPipelineView.settings from vscode-llvm!');
		let doc1 = await vscode.workspace.openTextDocument( ".vscode/llvm.json" );
		let edit1 = await vscode.window.showTextDocument( doc1, undefined, true );
	});

	context.subscriptions.push(disposable);
	context.subscriptions.push(open_settings);
}
